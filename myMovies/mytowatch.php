	  ...
	  <?php
	  	...
	  	if($_SERVER["REQUEST_METHOD"] == "POST") {
	  		...
	  		if($result0->num_rows == 0) {
	  			$stmt = $link->prepare("SELECT * FROM movies AS m JOIN user_movie_list AS u ON m.movieID=u.movieID WHERE userID = ? AND towatch = 1 AND (title=? OR releaseYear=? OR maleActor=? OR femaleActor=? OR Director=?)");
		    	...
		    	$result = $stmt->get_result();
	  			...
	  			$sql3 = "SELECT * from movies AS m  
	  					...
	  					WHERE genreID=".$row0["genreID"]." AND towatch = 1 AND userID = ".$_SESSION["id"];
		    ...
		    while($row = $result->fetch_assoc()) {
		        	...
		        	echo $temporary.'</div>
		        		...
		        		while($row2 = $result2->fetch_assoc()) {
		       			...
		       			echo 	'</h2>
						        ...
						        <form action="deleteToWatch.php" method="post">
						        	<input type="hidden" name="movieID" value="'.$row["movieID"].'">
						        	<input type="submit" value="Delete">
						        </form>
						        <form action="addToWatched.php" method="post">
						        	<input type="hidden" name="movieID" value="'.$row["movieID"].'">
						        	<input type="submit" value="Add To Watched">
						        </form>
	  		...
	  		$stmt = $link->prepare("SELECT * FROM movies AS m JOIN user_movie_list AS u ON m.movieID = u.movieID WHERE towatch = 1 AND userID = ?");
	  		$stmt->bind_param("s", $_SESSION["id"]);
		    $stmt->execute();
		    $result = $stmt->get_result();
