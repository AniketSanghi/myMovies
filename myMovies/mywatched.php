	  ...
	  <?php
	  	...
	  	if($_SERVER["REQUEST_METHOD"] == "POST") {
	  		...
	  		if($result0->num_rows == 0) {
	  			$stmt = $link->prepare("SELECT * FROM movies AS m JOIN user_movie_list AS u ON m.movieID=u.movieID WHERE userID = ? AND watched = 1 AND (title=? OR releaseYear=? OR maleActor=? OR femaleActor=? OR Director=?)");
	  			$stmt->bind_param("ssssss", $_SESSION["id"], $_POST["search"], $_POST["search"], $_POST["search"], $_POST["search"], $_POST["search"]);
		    	$stmt->execute();
		    	$result = $stmt->get_result();
	  		}
	  		else {
	  			$row0 = $result0->fetch_assoc();
	  			$sql3 = "SELECT * from movies AS m  
	  					JOIN movie_genre AS g ON m.movieID = g.movieID 
	  					JOIN user_movie_list AS u ON u.movieID = m.movieID
	  					WHERE genreID=".$row0["genreID"]." AND watched = 1 AND userID = ".$_SESSION["id"];
		    ...
		    while($row = $result->fetch_assoc()) {
		    	...
		    	$st1 = $link->prepare("SELECT rating as avg, review from user_movie_list WHERE watched = 1 AND movieID = ? AND userID = ?");
		    	$st1->bind_param("ss", $row["movieID"], $_SESSION["id"]);
		    	$st1->execute();
		        	...
		        	echo $temporary.'</div>
		        		...
		        		while($row2 = $result2->fetch_assoc()) {
		       			...
		       			echo 	'</h2>
						        ...
						        <form action="deleteWatched.php" method="post">
	  		...
	  		$stmt = $link->prepare("SELECT * FROM movies AS m JOIN user_movie_list AS u ON m.movieID = u.movieID WHERE watched = 1 AND userID = ?");
	  		$stmt->bind_param("s", $_SESSION["id"]);
		    $stmt->execute();
		    $result = $stmt->get_result();
